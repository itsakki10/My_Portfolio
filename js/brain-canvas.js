...
(function() {
  ...
  const orbitItems = [
    { text: '[ FUEL.caffeine ]', name: 'Caffeine', r: 140, speed: 0.009, phase: 0, color: '#fbbf24' },
    { text: '[ AGENT.synapse ]', name: 'AI Agents', r: 165, speed: -0.007, phase: 1.5, color: '#22d3ee' },
    { text: '[ STRATEGY.chess ]', name: 'Chess AI', r: 185, speed: 0.005, phase: 3.0, color: '#a78bfa' },
    { text: '[ VISION.vox ]', name: 'Computer Vision', r: 150, speed: -0.008, phase: 0.5, color: '#22d3ee' },
    { text: '[ MODEL.weights ]', name: 'Deep Learning', r: 185, speed: 0.007, phase: 2.2, color: '#fbbf24' },
    { text: '[ WIN.hackathon ]', name: 'Competitions', r: 210, speed: -0.004, phase: 4.5, color: '#a78bfa' },
    { text: '[ CORE.discipline ]', name: 'Athletics', r: 130, speed: 0.011, phase: 1.0, color: '#faf7f2' },
    { text: '[ MLOPS.deploy ]', name: 'System Design', r: 190, speed: -0.006, phase: 3.8, color: '#22d3ee' }
  ];
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    let startDragYaw = 0, startDragPitch = 0;
  ...
  function buildBrainNodes() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let colorPrefix;
      if (randColor > 0.3) {
        colorPrefix = 'rgba(251, 191, 36, '; // Amber warm glow (70%)
      } else if (randColor > 0.5) {
        colorPrefix = 'rgba(34, 211, 238, ';  // Cyber neon cyan (15%)
      } else {
    ...
    for (let i = 0; i < bgCount; i++) {
      const randColor = Math.random();
      const colorPrefix = randColor > 0.5 ? 'rgba(251, 191, 36, ' : 'rgba(34, 211, 238, ';
  ...
  function project(x, y, z) {
    ...
    const scale = (dist / (dist + z2 * zoomFactor)) * currentScale3D * zoomFactor;
  ...
  function draw() {
    ...
    projectedNodes.forEach(n => {
      ...
      ctx.fillStyle = n.colorPrefix + (pulseAlpha * fadeAlpha) + ')';
      ctx.fill();
    });
    ...
    orbits.forEach(item => {
      ...
      if (proj.z > -150) { // filter extreme back-clipping
        ctx.save();
